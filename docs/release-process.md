...
2. Run `npm run test:tooling` and
   `node scripts/verify-release-version.mjs <version>`, then complete the normal
   local checks and confirm CI on `main`.
