...
| AviationWeather.gov Data API | User approves and starts its provider plugin, then explicitly selects **Fetch airport weather** after importing a plan                    | Up to ten origin, destination, and alternate ICAO identifiers plus normal connection metadata                | Public service operated by the U.S. National Weather Service; current API host is `aviationweather.gov`                   | Do not approve or start the plugin, stop it, do not fetch weather, clear the session plan, or close the application | Reconfirm the public API contract, operating guidance, service notices, and provider status before each supported release                       |
| Open-Meteo Forecast API      | User approves and starts its provider plugin; host refreshes the coarse global model layer about every fifteen minutes                    | 84 host-selected global coordinates, requested field names, units, and normal connection metadata            | Independent weather API at `api.open-meteo.com`                                                                           | Do not approve or start the plugin, or stop it in Forge                                                             | Reconfirm API access, attribution, usage limits, and field contract before each supported release                                               |
| RainViewer Weather Maps API  | User approves and starts its provider plugin; host refreshes current metadata and four global zoom-one PNG tiles about every five minutes | Host-selected tile addresses and normal connection metadata                                                  | Independent weather service at `api.rainviewer.com` and `tilecache.rainviewer.com`                                        | Do not approve or start the plugin, or stop it in Forge                                                             | Reconfirm public API availability, attribution, caching, and reuse conditions before each supported release                                     |
