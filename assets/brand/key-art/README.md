...
This directory preserves the model-native candidate renders for WyrmGrid's
master promotional artwork. Files under `originals/` are immutable source
artifacts: do not resize, recompress, colour-correct, crop, or overwrite them.
Create any later production treatments in a sibling `derivatives/` directory.
