...
The desktop imports both hero derivatives through the `$brand` build alias and
selects between them from the active theme canvas luminance. The launch surface
keeps the artwork visible for a short bounded interval while local settings are
loaded. The frontend post-build check verifies that both exact source checksums
are present in the packaged static application.
