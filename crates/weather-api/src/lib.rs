...
mod tests {
    ...
    fn rejects_malformed_and_oversized_payloads_without_echoing_them() {
        ...
        let error = translate_airport_weather(&["YSSY".into()], canary, TAF_FIXTURE, Utc::now())
            .unwrap_err();
    ...
    async fn requests_only_bounded_documented_json_products_without_authentication() {
        ...
        assert!(
            requests[0]
                .starts_with("GET /api/data/metar?ids=NZAA%2CNZWN%2CYSSY&format=json HTTP/1.1")
        );
        assert!(
            requests[1]
                .starts_with("GET /api/data/taf?ids=NZAA%2CNZWN%2CYSSY&format=json HTTP/1.1")
        );
        assert!(
            requests
                .iter()
